...
IG_EDIT_PROFILE = "{{\"gender\":\"3\",\"username\":\"{}\",\"first_name\":\"bruh\",\"biography\":\"Welcome home!\",\"email\":\"{}\"}}"
...
class Instagram(object):
	...
	def build_claim_data(self):
		self.check_url = "{}/feed/user/{}/reel_media/".format(self.url, self.target)
...
def twilio(sms_to, message):
	phone_number = "+441751242051" # Twilio phone number
	sid = "AC39775351d589231e176d904f0acf0c84" # Twilio SID
	auth = "ed3d285cf3aaeea1147fc7e02a7c6510" # Twilio auth token

	try:
		Client(sid, auth).messages.create(
			to=sms_to, from_=phone_number, body=message
		)
	except:
		return False
...
def main():
	...
	sms_number = None
	sms = get_input("\r\n{} Would you like SMS notifications? [y/n]: ".format(INPUT)).strip().lower()
	clear = " ".join(" " for _ in range(len(sms)))

	if (sms == "y" or sms == "yes"):
		print("\x1b[A                                             {}\x1b[A".format(clear))
		sms_number = get_input("{} Phone number: ".format(INPUT)).strip()
		clear = " ".join(" " for _ in range(len(sms_number)))
		print("\x1b[A                  {}\x1b[A".format(clear))
	else:
		print("\x1b[A                                             {}\x1b[A".format(clear))
	...
	while (instagram.running):
		try:
			for spinner in ["|", "/", "-", "\\", "|", "/", "-", "\\"]:
				...
				sleep(0) # Update attempts every 100ms
	...
	if (instagram.spam_blocked):
		...
		if (sms_number is not None):
			twilio(sms_number, "Tried to claim @{} but account is spam blocked ({:,} attempts).".format(instagram.target, instagram.attempts))

	elif (instagram.rate_limited):
		print("\r{} Rate limited after ({:,} attempts)\r\n".format(ERROR, instagram.attempts))

		if (sms_number is not None):
			twilio(sms_number, "Rate limited after ({:,} attempts).".format(instagram.attempts))

	elif (instagram.missed_swap):
		print("\r{} Missed username swap on @{} after {:,} attempts\r\n".format(ERROR, instagram.target, instagram.attempts))

		if (sms_number is not None):
			twilio(sms_number, "Missed username swap on @{} after {:,} attempts.".format(instagram.target, instagram.attempts))
